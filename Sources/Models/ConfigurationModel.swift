//
// ConfigurationModel.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public struct ConfigurationModel: Codable, JSONEncodable, Hashable {

    public var description: String?
    public var parameters: [String: String]?
    /** This property is used to link configuration with project */
    public var projectId: UUID
    public var isDefault: Bool
    public var name: String?
    public var createdDate: Date
    public var modifiedDate: Date?
    public var createdById: UUID
    public var modifiedById: UUID?
    public var globalId: Int64
    /** Unique ID of the entity */
    public var id: UUID
    /** Indicates if the entity is deleted */
    public var isDeleted: Bool

    public init(description: String? = nil, parameters: [String: String]? = nil, projectId: UUID, isDefault: Bool, name: String? = nil, createdDate: Date, modifiedDate: Date? = nil, createdById: UUID, modifiedById: UUID? = nil, globalId: Int64, id: UUID, isDeleted: Bool) {
        self.description = description
        self.parameters = parameters
        self.projectId = projectId
        self.isDefault = isDefault
        self.name = name
        self.createdDate = createdDate
        self.modifiedDate = modifiedDate
        self.createdById = createdById
        self.modifiedById = modifiedById
        self.globalId = globalId
        self.id = id
        self.isDeleted = isDeleted
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case description
        case parameters
        case projectId
        case isDefault
        case name
        case createdDate
        case modifiedDate
        case createdById
        case modifiedById
        case globalId
        case id
        case isDeleted
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(description, forKey: .description)
        try container.encodeIfPresent(parameters, forKey: .parameters)
        try container.encode(projectId, forKey: .projectId)
        try container.encode(isDefault, forKey: .isDefault)
        try container.encodeIfPresent(name, forKey: .name)
        try container.encode(createdDate, forKey: .createdDate)
        try container.encodeIfPresent(modifiedDate, forKey: .modifiedDate)
        try container.encode(createdById, forKey: .createdById)
        try container.encodeIfPresent(modifiedById, forKey: .modifiedById)
        try container.encode(globalId, forKey: .globalId)
        try container.encode(id, forKey: .id)
        try container.encode(isDeleted, forKey: .isDeleted)
    }
}


@available(iOS 13, tvOS 13, watchOS 6, macOS 10.15, *)
extension ConfigurationModel: Identifiable {}
