...
internal typealias ruTestitApiClientAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, ruTestitApiClientAPIChallengeHandler>()
...
internal class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    internal var taskDidReceiveChallenge: ruTestitApiClientAPIChallengeHandler?
    ...
    @discardableResult
    override internal func execute(_ apiResponseQueue: DispatchQueue = ruTestitApiClientAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    internal func buildHeaders() -> [String: String] {
        ...
        for (key, value) in ruTestitApiClientAPI.customHeaders {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.ruTestitApiClient.\(UUID().uuidString)")
                }
